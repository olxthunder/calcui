/* ============================================
  ...
  function updateFontSizes() {
    // Error стан — не змінюємо розмір, показуємо окремий лейбл
    if (state.current === 'Error') return;
  
    const len = state.current.length;
    resultEl.classList.remove('display__result--md', 'display__result--sm', 'display__result--xs');
    if      (len >= 13) resultEl.classList.add('display__result--xs');
    else if (len >= 10) resultEl.classList.add('display__result--sm');
    else if (len >= 7)  resultEl.classList.add('display__result--md');
  ...
  function render() {
    ...
    // Дисплей: результат і вираз
    expressionEl.textContent = state.expression;
  
    if (isError) {
      // В error стані — показуємо порожній результат і лейбл
  ...
  function inputBackspace() {
    if (state.justCalculated || state.current === 'Error') { inputClear(); return; }
